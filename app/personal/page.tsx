          ...
          {personalData.map((d) => (
            <div key={d.title} className="md p-4 md:w-1/2" style={{ maxWidth: '544px' }}>
              <div className="border-opacity-60 overflow-hidden rounded-md border-2 border-gray-200 dark:border-gray-700">
                ...
                {/* 文字描述 */}
                <div className="p-6">
                  <h2 className="mb-3 text-2xl leading-8 font-bold tracking-tight">{d.title}</h2>
                  ...
                  {/* 新增的列表渲染逻辑 */}
                  {d.list && (
                    <ul className="mt-2 grid list-disc grid-cols-1 gap-x-4 gap-y-1 pl-5 text-sm text-gray-500 sm:grid-cols-2 dark:text-gray-400">
